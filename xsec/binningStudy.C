...
// must contain ngroup+1 elements that represent the low-edges of the bins. If
// the original histogram has errors stored (via Sumw2), the resulting
...
void binningStudy(int signal_definition_int = 0) {
  ...
  ContainerEraser::erase_if(
      variables, [](Variable* v) { return v->Name() == "wexp_fit"; });
  ...
  for (auto var : variables) {
    ...
    PlotMC(selection, plot_info, Form("Selection_%s", var->Name().c_str()), -1.,
           "N Events");
    PlotStatError(selection, plot_info,
                  Form("StatError_%s", var->Name().c_str()), -1.,
                  "stat error (%)");
    ...
    /*
    {
      ...
      PlotUtils::MnvH1D* hist = (MnvH1D*)selection->Rebin(nbins, "",
    rebins_array);

      // after rebinning
      //PlotMC(hist, plot_info, Form("Selection_%s_rebin", var->Name().c_str()),
    -1., "N Events"); PlotStatError(hist, plot_info, Form("StatError_%s_rebin",
    var->Name().c_str()), -1., "stat error (%)"); Plot_ErrorSummary(plot_info,
    hist, "SelRebin");

      // after rebinning + bin width norm
      hist->Scale(1., "width");
      //PlotMC(hist, plot_info, Form("Selection_%s_rebin_bwn",
    var->Name().c_str()), -1., "N Events / MeV");
      //PlotStatError(hist, plot_info, Form("StatError_%s_rebin_bwn",
    var->Name().c_str()), -1., "stat error (%)");
    ...
    */
    ...
    // PlotMC(eff,              plot_info, Form("Efficiency_%s",
    // var->Name().c_str()), 0.2, "Efficiency");
