...
void GXSEClosure(int signal_definition_int = 0) {
  ...
  for (auto v2U : Vars2Use) {
    for (auto var : variables) {
      ...
      Variable* true_var =
          GetVar(variables, var->Name() + std::string("_true"));
      ...
      mnv_unfold.UnfoldHisto(reco_var->m_hists.m_unfolded, migration,
                             reco_sel_mc, RooUnfold::kBayes, n_iterations);
      ...
      double n_target_nucleons =
          PlotUtils::TargetUtils::Get().GetTrackerNNucleons(
      ...
      mc_integrate_flux->Scale(n_target_nucleons * util.m_mc_pot / normFactor);
      ...
      if (var->Name() == v2U) {
        ...
        PlotUtils::MnvH1D* pmu_xsec_dummy =
            (PlotUtils::MnvH1D*)fin_gxse.Get(Form("%s_xsec", v2U.c_str()));
        ...
        for (int i = 0; i < pmu_xsec->GetNbinsX() + 1; ++i) {
          std::cout << i << "  " << pmu_xsec_dummy->GetBinLowEdge(i) << "  "
                    << pmu_xsec_dummy->GetBinContent(i) / 1000
                    << "  "  // it for the binning normalization
      ...
      if (var->Name() == v2U) {
        ...
        PlotUtils::MnvH1D* pmu_rate_dummy = (PlotUtils::MnvH1D*)fin_gxse.Get(
            Form("%s_xsec_evRate", v2U.c_str()));
