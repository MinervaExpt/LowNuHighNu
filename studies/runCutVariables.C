...
std::vector<Variable*> GetOnePiVariables(bool include_truth_vars = false) {
  ...
  Var* wexp_cut = new Var("wexp_cut", "W_{exp}", "MeV", 20, 0., 2.0e3,
                          &CVUniverse::GetWexp);
  Var* michel_count = new Var("michel_count", "michel_count", "", 5, 0., 5.);

  HVar* llr =
      new HVar("llr", "LLR", "", 40, -60., 60., &CVUniverse::GetLLRScore);
  HVar* enode01 =
      new HVar("enode01", "Enode01", "", 70, 0., 70., &CVUniverse::GetEnode01);
  HVar* enode2 =
      new HVar("enode2", "Enode2", "", 40, 0., 40., &CVUniverse::GetEnode2);
  HVar* enode3 =
      new HVar("enode3", "Enode3", "", 40, 0., 40., &CVUniverse::GetEnode3);
  HVar* enode4 =
      new HVar("enode4", "Enode4", "", 40, 0., 40., &CVUniverse::GetEnode4);
  HVar* enode5 =
      new HVar("enode5", "Enode5", "", 40, 0., 40., &CVUniverse::GetEnode5);

  Var* n_iso_prongs = new Var("n_iso_prongs", "n_iso_prongs", "", 5, 0., 5.,
                              &CVUniverse::GetNIsoProngs);
  Var* n_pions = new Var("n_pions", "NPions", "", 5, 0, 5.);

  Var* fit_vtx_x = new Var("fit_vtx_x", "fit_vtx_x", "cm", 50, -100, 100,
                           &CVUniverse::GetFitVtxX);
  Var* fit_vtx_y = new Var("fit_vtx_y", "fit_vtx_y", "cm", 50, -100, 100,
                           &CVUniverse::GetFitVtxY);
  Var* fit_vtx_z = new Var("fit_vtx_z", "fit_vtx_z", "cm", 50, -1000, 1000,
                           &CVUniverse::GetFitVtxZ);

  Var* wexp0 =
      new Var("wexp0", "W_{exp}", "MeV", 20, 0., 2.0e3, &CVUniverse::GetWexp);
  Var* wexp1 =
      new Var("wexp1", "W_{exp}", "MeV", 20, 0., 2.0e3, &CVUniverse::GetWexp);
  Var* wexp2 =
      new Var("wexp2", "W_{exp}", "MeV", 20, 0., 2.0e3, &CVUniverse::GetWexp);
  Var* wexp3 =
      new Var("wexp3", "W_{exp}", "MeV", 20, 0., 2.0e3, &CVUniverse::GetWexp);
  Var* wexp4 =
      new Var("wexp4", "W_{exp}", "MeV", 20, 0., 2.0e3, &CVUniverse::GetWexp);
  Var* wexp5 =
      new Var("wexp5", "W_{exp}", "MeV", 20, 0., 2.0e3, &CVUniverse::GetWexp);
  Var* wexp6 =
      new Var("wexp6", "W_{exp}", "MeV", 20, 0., 2.0e3, &CVUniverse::GetWexp);
  Var* wexp7 =
      new Var("wexp7", "W_{exp}", "MeV", 20, 0., 2.0e3, &CVUniverse::GetWexp);
  Var* wexp8 =
      new Var("wexp8", "W_{exp}", "MeV", 20, 0., 2.0e3, &CVUniverse::GetWexp);
  Var* wexp9 =
      new Var("wexp9", "W_{exp}", "MeV", 20, 0., 2.0e3, &CVUniverse::GetWexp);

  std::vector<Var*> variables = {
      n_had_tracks, wexp_cut, michel_count, llr,     enode01, enode2, enode3,
      enode4,       enode5,   n_iso_prongs, n_pions, wexp0,   wexp1,  wexp2,
      wexp3,        wexp4,    wexp5,        wexp6,   wexp7,   wexp8,  wexp9,
  };
...
void runCutVariables(int signal_definition_int = 0,
  ...
  std::vector<Variable*> cut_variables =
      GetCutVariables(util.m_signal_definition, do_truth_vars);
  // std::vector<Variable*> cut_variables;

  std::vector<Variable*> ana_variables =
      GetAnalysisVariables(util.m_signal_definition, do_truth_vars);

  std::vector<Variable*> variables;

  variables.reserve(cut_variables.size() +
                    ana_variables.size());  // preallocate memory
