...
std::vector<Variable*> GetVariables() {
  ...
  Var* ecalrecoil = new Var("ecalrecoil", "ecalrecoil", "mev", necalbins,
                            ecalmin, ecalmax, &CVUniverse::GetCalRecoilEnergy);
  Var* ecalrecoil_ccinc =
      new Var("ecalrecoil_ccinc", "ecalrecoil_ccinc", "mev", necalbins, ecalmin,
              ecalmax, &CVUniverse::GetCalRecoilEnergy_CCIncSpline);
  Var* ecalrecoil_ccpi =
      new Var("ecalrecoil_ccpi", "ecalrecoil_ccpi", "mev", necalbins, ecalmin,
              ecalmax, &CVUniverse::GetCalRecoilEnergy_CCPiSpline);
  Var* ecalrecoil_default =
      new Var("ecalrecoil_default", "ecalrecoil_default", "mev", necalbins,
              ecalmin, ecalmax, &CVUniverse::GetCalRecoilEnergy_DefaultSpline);

  // Ecal - nopi
  Var* ecalrecoilnopi = new Var(
      "ecalrecoilnopi", "ecalrecoilnopi", "mev", CCPi::GetBinning("ecal_nopi"),
      &CVUniverse::GetCalRecoilEnergyNoPi_DefaultSpline);
  Var* ecalrecoilnopi_ccinc =
      new Var("ecalrecoilnopi_ccinc", "ecalrecoilnopi_ccinc", "mev",
              CCPi::GetBinning("ecal_nopi"),
              &CVUniverse::GetCalRecoilEnergyNoPi_CCIncSpline);
  Var* ecalrecoilnopi_corr =
      new Var("ecalrecoilnopi_corr", "ecalrecoilnopi_corr", "mev",
              CCPi::GetBinning("ecal_nopi"));
  Var* ecalrecoilnopi_true = new Var(
      "ecalrecoilnopi_true", "ecalrecoilnopi_true", "mev",
      CCPi::GetBinning("ecal_nopi"), &CVUniverse::GetCalRecoilEnergyNoPiTrue);
  ...
  Var* etrackrecoil =
      new Var("etrackrecoil", "etrackrecoil", "mev", netracksbins, etracksmin,
              etracksmax, &CVUniverse::GetTrackRecoilEnergy);
  Var* etracks_true =
      new Var("etracks_true", "etracks_true", "mev", netracksbins, etracksmin,
              etracksmax, &CVUniverse::GetAllTrackEnergyTrue);
  ...
  HVar* epi_cal = new HVar("epi_cal", "epi_cal", "mev", nepibins, epimin,
                           epimax, &CVUniverse::GetCalEpi);
  HVar* epi_true = new HVar("epi_true", "epi_true", "mev", nepibins, epimin,
                            epimax, &CVUniverse::GetEpiTrueMatched);
  ...
  Var* wexp =
      new Var("wexp", "wexp", "mev", nwbins, wmin, wmax, &CVUniverse::GetWexp);
  Var* wexp_true = new Var("wexp_true", "wexp_true", "mev", nwbins, wmin, wmax,
                           &CVUniverse::GetWexpTrue);
  Var* wgenie = new Var("wgenie", "wgenie", "mev", nwbins, wmin, wmax,
                        &CVUniverse::GetWgenie);
  Var* wresid = new Var("wresid", "wresid", "mev", 30, -0.5, 0.5,
                        &CVUniverse::GetWexpFResidual);
...
void runRecoilEnergy(std::string plist = "ME1A") {
  ...
  for (auto v : variables) {
    ...
    std::cout << tag << "\n";
    double ymax = -1;
    if (tag == "wexp" || tag == "wexp_true") ymax = 1.25;
    if (tag == "etrackrecoil" || tag == "etracks_true") ymax = 3.1;
    if (tag == "epi_cal" || tag == "epi_true") ymax = 1.7;
    if (tag == "ehad" || tag == "ehad_true") ymax = 0.9;
    if (tag == "ecalrecoil_ccinc" || tag == "ecalrecoil_default" ||
        tag == "ecalrecoil_ccpi")
      ymax = 0.9;
    if (tag == "wresid") ymax = 1.7e3;
    ...
    if (!v->m_is_true) {
      double zmax = -1;
      if (tag == "ecalrecoilnopi_corr") zmax = 50;
      PlotUtils::MnvH2D* mig =
          (PlotUtils::MnvH2D*)v->m_hists.m_migration.hist->Clone(uniq());
