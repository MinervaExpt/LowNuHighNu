...
enum FSParticleType {
  ...
  kCCnumu_0pip_NpiX,
  kCCnumu_2pip_NpiX,
  kCCnumu_1pip_NpiX,  // BGs
  kCCnumu_2pip_0piX,
  kCCnumu_1pip_0piX,  // Signals
...
enum ChannelType { kCCQE, kCOHPI, kNONRES, kRES, kNChannelTypes };
...
FSParticleType GetFSParticleType(const CVUniverse& universe) {
  ...
  // 1pi+, 0 or N piX
    ...
    if (N_pim == 0 && N_pi0 == 0)
      return kCCnumu_1pip_0piX;
    else
      return kCCnumu_1pip_NpiX;
  }

  // Npi+, 0 or N piX
  else if (N_pip == 2) {
    if (N_pim == 0 && N_pi0 == 0)
      return kCCnumu_2pip_0piX;
    else
      return kCCnumu_2pip_NpiX;
  }
...
std::string GetTruthClassification_LegendLabel(
    FSParticleType interaction_category) {
...
// kNoInteraction=0, kQEL=1, kRES=2, kDIS=3, kCOHPI=4, kAMNUGAMMA=5, kIMD=6,
// kNUEEL=7, k2P2H=8
...
std::string GetTruthClassification_LegendLabel(
    ChannelType interaction2_category) {
