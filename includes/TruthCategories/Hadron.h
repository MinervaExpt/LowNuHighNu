...
HadronType GetHadronType(const CVUniverse& universe) {
  ...
  int destruct_code =
      universe.GetVecElem("MasterAnaDev_hadron_tm_destructCode", 1);
  // std::cout << hadron_id << " " << pdg << " " <<  destruct_code << std::endl;
  // double frac = universe.GetVecElem("MasterAnaDev_hadron_tm_fraction",
  // hadron_id); if (frac < 0.5)
  //  std::cout << "frac = " << frac << std::endl;

  // int aaron_pdg = universe.GetVecElem("MasterAnaDev_hadron_PDGCode",
  // hadron_id); // Identical to other method.
...
std::string GetTruthClassification_LegendLabel(HadronType hadron_category) {
  switch (hadron_category) {
    case kStoppingPip:
      return "Stopping #pi^{+}";
    case kInteractPip:
      return "Interacting #pi^{+}";
    case kPim:
      return "#pi^{-}";
    case kStoppingP:
      return "Stopping Proton";
    case kInteractP:
      return "Interacting Proton";
