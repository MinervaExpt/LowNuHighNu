...
double getResurrectionMCFilePOT(const char* filename) {
  ...
  //
  // so first split at underscores, then get the 5th entry and split it at
  // dashes
  ...
  if (check->GetString() != TString("Subruns")) {
    cout << "Can't parse " << basename
         << " because item ought to be Subruns but is " << check << endl;
    exit(1);
  }
...
void setBranchStatuses(TChain& ch) {
  // In latest ntuples, there aren't any really useless branches, so we'll just
  // keep everything
  return;
