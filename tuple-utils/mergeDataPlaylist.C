...
// only merges one run.
void mergeDataPlaylist(const char* inDirBase, const char* outDir,
                       const char* playlist, const char* tag = "MECAnaTool",
                       const char* treeName = "MECAna") {
  //******************************************************************
  //* Set Location of output files
  //******************************************************************
  TString output =
      TString::Format("%s/%s_merged_%s.root", outDir, playlist, treeName);
  ...
  // 7 -> 7B happens in the middle of a run, so just put the runs in minerva7 to
  // avoid double-counting playlistRunRanges["minerva7B"]=make_pair(3152, 3160
  // );
  ...
  for (int run = minrun; run <= maxrun; ++run) {
    ...
    TString inGlob(TString::Format(
        "%s/%s/%s/%s/%s/MV_*%s_*_%s*.root", inDirBase, runStrParts[0].c_str(),
        runStrParts[1].c_str(), runStrParts[2].c_str(), runStrParts[3].c_str(),
        runStr.c_str(), tag));
