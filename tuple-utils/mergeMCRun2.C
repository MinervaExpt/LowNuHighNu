...
// only merges one run.
void mergeMCRun2(const char* inDirBase, const char* outDir, int run,
                 const char* tag = "CCNuPionInc",
                 const char* treeName = "CCNuPionInc") {
  //******************************************************************
  //* Set Location of output files
  //******************************************************************
  TString output =
      TString::Format("%s/merged_%s_run%08d.root", outDir, tag, run);
  ...
  TString inGlob(TString::Format("%s/%s/%s/%s/%s/SIM_*%s_*_%s*.root", inDirBase,
                                 runStrParts[0].c_str(), runStrParts[1].c_str(),
                                 runStrParts[2].c_str(), runStrParts[3].c_str(),
                                 runStr.c_str(), tag));
  ...
  cout << "Added " << nFiles << " files out of " << nFilesTotal << " in run "
       << run << endl;
  ...
  fout->cd();
  TTree* outTreeTruth =
      inChainTruth.CopyTree("mc_vtx[2]>5891 && mc_vtx[2]<8439");
  outTreeTruth->Write();
