...
// only merges one run.
void mergeDataFiles(const char* inDirBase, const char* outDir,
                    const char* tag = "MasterAnaDev") {
  //******************************************************************
  ...
  // TString
  // inGlob(TString::Format("%s/*/*/*/*/MV_00016523_Subruns_000*%s*.root",
  // inDirBase, tag));

  cout << "Filename glob is " << inGlob << endl;
  cout << "Output filename is " << output << endl;
  const std::string XROOTD_PREFIX =
  ...
  for (int i = 0; i < (int)g.gl_pathc; ++i) {
    ...
    tup = XROOTD_PREFIX +
          tup.substr(5, tup.length());  // replace "/pnfs/" with xrootd prefix
